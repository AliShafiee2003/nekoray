...
#define MAKE_SETTINGS_STREAM_SETTINGS                         \
    ...
    if (streamSettings.contains("_unsupported_error")) {      \
        result.error = streamSettings["_unsupported_error"].toString(); \
        return result;                                        \
    }                                                         \
...
namespace NekoGui_fmt {
    QJsonObject V2rayStreamSettings::BuildStreamSettingsV2Ray() {
        // Reject transport types not supported by Xray 1.8.4
        if (network == "xhttp" || network == "splithttp" || network == "kcp") {
            QJsonObject errorResult;
            errorResult["_unsupported_error"] = QString("Unsupported transport type: %1 (not supported by Xray core)").arg(network);
            return errorResult;
        }
